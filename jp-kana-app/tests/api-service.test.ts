...
import { getRandomKana, getKanaPerformance, recordKanaPerformance, getKanaWithWeights } from '../src/lib/api-service';
...
describe('API Service', () => {
  ...
  describe('recordKanaPerformance', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(recordKanaPerformance('test-user', 'あ', 'hiragana', true)).rejects.toThrow();
    });
  });
  ...
  describe('getKanaWithWeights', () => {
    it('should fetch kana with weights successfully', async () => {
      ...
      const result = await getKanaWithWeights(characters, 'test-user', 'hiragana');
    ...
    it('should handle API errors', async () => {
      ...
      await expect(getKanaWithWeights(characters, 'test-user', 'hiragana')).rejects.toThrow();
    });
  });
}); 